...
std::array<unsigned char, SHA256_DIGEST_LENGTH> row_hash(
    ...
    if (::SHA256(
        ...
        throw std::runtime_error("A128 SHA-256");
    return digest;
}
...
std::vector<float> fit_residuals(
    ...
    if (learn.size() != assignments.size() * kHead ||
        ...
        rows.size() != kFitRows)
        throw std::invalid_argument("A128 fit shape");
    ...
    for (std::size_t index = 0; index < kFitRows; ++index) {
        ...
        if (row >= assignments.size() ||
            ...
            throw std::out_of_range("A128 selected row");
...
void write_shape(
        ...
        const mixedradix::Shape& shape) {
    if (shape.radices.size() != shape.used_states.size())
        throw std::invalid_argument("A128 shape sidecar");
    std::ofstream output(path, std::ios::binary | std::ios::trunc);
    for (std::size_t group = 0;
         group < shape.radices.size(); ++group) {
        output.write(
    ...
    if (!output ||
        std::filesystem::file_size(path) !=
                shape.radices.size() * 2 * sizeof(std::uint16_t))
        throw std::runtime_error("write A128 shape");
}

void write_resources(
        const std::filesystem::path& output,
        const std::string& id, std::size_t nlist,
        double fit_cpu, double fit_wall,
    ...
    record << "arm_id\tdimensions\tnlist\tfit_rows\tbase_rows"
              ...
              "\tfit_cpu_seconds\tfit_wall_seconds"
    ...
    record << std::setprecision(17)
           ...
           << (id.ends_with("B4") ? 32 : 64) << '\t'
           << fit_cpu << '\t' << fit_wall << '\t'
           << add_cpu << '\t' << add_wall << '\t'
           << write_cpu << '\t' << write_wall << '\t'
           << load_cpu << '\t' << load_wall << '\t'
           << peak_rss_bytes() << '\n';
    if (!record) throw std::runtime_error("write A128 resources");
}
...
void run(
    ...
    if ((!sift && dataset != "gist") ||
        ...
        (bits != 4 && bits != 8))
        throw std::invalid_argument("A128 arguments");
    if (std::filesystem::exists(output))
        throw std::invalid_argument("A128 output exists");
    const std::size_t learn_rows = sift ? 100000 : 500000;
    const std::filesystem::path view =
            sift ? common :
            std::filesystem::path(common.string() + "_head128");
    ...
    double fit_cpu = 0, fit_wall = 0;
    a4orb::Model model;
    {
        const Timer timer;
        model = a4orb::allocate_scalar(
                a4orb::fit_curves(fit, 1024), bits, false);
        std::tie(fit_cpu, fit_wall) = elapsed(timer);
    }
    const auto evaluation = a4orb::evaluate(fit, model);
    if (!evaluation.shape_valid ||
        !evaluation.encoding_valid ||
        evaluation.collisions != 0)
        throw std::runtime_error("A128 model validity");
    ...
    double write_cpu = 0, write_wall = 0;
    {
        ...
        write_shape(
                output.string() + ".shape.u16",
                mixedradix::shape_of(model));
    ...
    double load_cpu = 0, load_wall = 0;
    {
        ...
        const auto* loaded =
                dynamic_cast<const faiss::IndexIVFPQ*>(
                        loaded_owner.get());
        if (loaded == nullptr ||
            loaded->pq.centroids != index->pq.centroids)
            throw std::runtime_error("A128 load parity");
        mixedradix::validate_codes(
                *loaded, mixedradix::shape_of(model));
        std::tie(load_cpu, load_wall) = elapsed(timer);
    }
    write_resources(
            output, "A128_B" + std::to_string(bits), nlist,
            fit_cpu, fit_wall, add_cpu, add_wall,
    ...
    try {
        if (argc != 7)
            throw std::invalid_argument(
                    "usage: mixed_radix_build_a_arm "
                    "<sift|gist> <1024|4096> <4|8> "
                    "<common_root> <output.faiss> <frozen>");
        if (std::string(argv[6]) != "frozen")
            throw std::invalid_argument("missing frozen marker");
        run(
                argv[1], std::stoull(argv[2]), std::stoi(argv[3]),
                argv[4], argv[5]);
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "mixed_radix_build_a_arm: "
                  << error.what() << '\n';
        return 1;
    }
}
