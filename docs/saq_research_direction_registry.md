...
| `R12` | 2026-07-13 draft Attempt 4 | Fixed-rate arbitrary-cardinality mixed-radix scalar-product quantization | `saq-arbitrary-cardinality-feasibility-v2@71e6bec` | `AWAITING_AUTHORIZATION` | preserve terminal A4-1S `NO_GO_EXACT_SOLVER_COST` and reviewed A4-V2-PAR `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; CACHE-I remains `GENERIC_CACHE_POLICY_SOURCE_STATIC_REVIEW_PASS`; PREP authorization target/review `e7f940e/@16a8201` reached only `PREP_AUTHORIZATION_EXACT_TARGET_REVIEW_PASS`; a static prelaunch host mismatch then prevented PREP and START, and host-rebind erratum target/review `5a47fed/@b89dabe` reached only `HOST_IDENTITY_REBIND_PROTOCOL_REVIEW_PASS`; HOST-I-AUTH target/review `212a67b/@71e6bec` then reached only `AUTHORIZATION_EXACT_TARGET_REVIEW_PASS`; no five-source/seven-derived-object rebind occurred, HOST-I remains `NOT_AUTHORIZED`, host identity is not rebound, and Python/PREP remain `NOT_AUTHORIZED_NOT_RUN`; the old invocation authority is unspent but nontransferable, and the old probe is unspent but superseded and may never run or be reused; fresh PREP authorization, actual PREP, CACHE-BIND, corrected PAR, and scientific execution remain unauthorized | `UNREPORTED` | `ABSENT` | `INCLUDED_PENDING_REPORT`, slides 59--65 |
...
| SAQ | `saq-arbitrary-cardinality-feasibility-v2` | `71e6bec` | `FOLLOWUP` | `R12` | host-rebind erratum target/review `5a47fed/@b89dabe` reached only `HOST_IDENTITY_REBIND_PROTOCOL_REVIEW_PASS`; HOST-I-AUTH target/review `212a67b/@71e6bec` then reached only `AUTHORIZATION_EXACT_TARGET_REVIEW_PASS`; HOST-I remains unauthorized, host identity is not rebound, Python/PREP were not run, the old authority is nontransferable, and the old probe is superseded; fresh PREP authorization, actual PREP, CACHE-BIND, PAR-R1, SRUN, data, quarantine, and SAQ work remain unauthorized |
| SAQ | `saq-meeting-summary` | `SELF` -- commit containing this registry; parent at reconciliation `e60f3b2` | `SUMMARY` | all | summary-only; no experiment execution |
