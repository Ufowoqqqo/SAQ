...
| `R12` | 2026-07-13 draft Attempt 4 | Fixed-rate arbitrary-cardinality mixed-radix scalar-product quantization | `saq-arbitrary-cardinality-feasibility-v2@e48df452` | `AWAITING_AUTHORIZATION` | preserve terminal A4-1S `NO_GO_EXACT_SOLVER_COST` and reviewed A4-V2-PAR `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; A4-V2-I-R1 repaired the running-CPython identity source and passed exact-commit static review only; bytecode-cache determinism and handling of quarantined staging/cache residuals remain unresolved, and PAR-R1 is not authorized | `UNREPORTED` | `ABSENT` | `INCLUDED_PENDING_REPORT`, slides 59--65 |
...
| SAQ | `saq-arbitrary-cardinality-feasibility-v2` | `c401dae` | `FOLLOWUP` | `R12` | exact source repair `e48df452` passed static review only; historical PAR remains `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; cache policy, PAR-R1, SRUN, data, and SAQ work remain unauthorized |
| SAQ | `saq-meeting-summary` | `SELF` -- commit containing this registry; parent at reconciliation `7ff83a4` | `SUMMARY` | all | summary-only; no experiment execution |
