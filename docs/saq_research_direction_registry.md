...
| `R12` | 2026-07-13 draft Attempt 4 | Fixed-rate arbitrary-cardinality mixed-radix scalar-product quantization | `saq-arbitrary-cardinality-feasibility-v2@e98a3e4` | `AWAITING_AUTHORIZATION` | preserve terminal A4-1S `NO_GO_EXACT_SOLVER_COST` and reviewed A4-V2-PAR `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; CACHE-I remains `GENERIC_CACHE_POLICY_SOURCE_STATIC_REVIEW_PASS`; PREP authorization and both host-protocol governance stages remain documentation-only; after HOST-I-AUTH target/review `212a67b/@71e6bec`, the first HOST-I instruction stopped before a source target on the seven-versus-eight component contradiction, additive source-authority erratum target/review `6fe8544/@9fa9528` reached only `HOST_I_SOURCE_AUTHORITY_ERRATUM_REVIEW_PASS`, and the subsequent 14-path HOST-I target/review `4602585/@e8e9c79` remained failed evidence with one HIGH false governing-review identity; bounded correction-only repair protocol target/review `ddfef99/@b1a7429` reached `HOST_I_R1_CORRECTION_ONLY_REPAIR_PROTOCOL_REVIEW_PASS`; the separately authorized exact five-path R1 target/review `e17f887/@e10bde7` then reached `HOST_I_R1_CORRECTION_ONLY_REPAIR_REVIEW_PASS` with zero LOW-or-higher findings after independently reproducing the four equal-length artifact substitutions, unchanged source/authority closure, and 13/13 registered host objects; `HOST_IDENTITY_REBOUND = ESTABLISHED_FOR_REGISTERED_13_OBJECT_BUNDLE_ONLY`, while whole-host identity remains unestablished; bounded source-history protocol target/review `dcaed57/@e98a3e4` reached `SOURCE_HISTORY_EPOCH_CORRECTION_PROTOCOL_REVIEW_PASS` with zero LOW-or-higher findings after independently proving the exact five-blob legacy closure and strict four-legacy/six-active role partition; this is protocol-only, so `SOURCE_HISTORY_MISMATCH` remains unresolved in source and cache-verifier/PAR readiness remains unestablished; no Python, build, PREP, data, or scientific execution occurred, and `A4-V2-SOURCE-HISTORY-I`, fresh PREP authorization, actual PREP, CACHE-BIND, corrected PAR, and scientific execution remain unauthorized | `UNREPORTED` | `ABSENT` | `INCLUDED_PENDING_REPORT`, slides 59--65 |
...
| SAQ | `saq-arbitrary-cardinality-feasibility-v2` | `e98a3e4` | `FOLLOWUP` | `R12` | HOST-I source-static target/review `4602585/@e8e9c79` remains failed evidence; R1 target/review `e17f887/@e10bde7` established rebound only for the registered 13-object bundle; source-history protocol target/review `dcaed57/@e98a3e4` reached `SOURCE_HISTORY_EPOCH_CORRECTION_PROTOCOL_REVIEW_PASS`, but the source correction remains unauthorized and unimplemented; whole-host identity remains unestablished, `SOURCE_HISTORY_MISMATCH` remains open in source, and Python/build/PREP/data were not run |
| SAQ | `saq-meeting-summary` | `SELF` -- commit containing this registry; parent at reconciliation `773823f` | `SUMMARY` | all | summary-only; no experiment execution |
