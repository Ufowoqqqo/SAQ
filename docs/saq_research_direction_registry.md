...
| `R12` | 2026-07-13 draft Attempt 4 | Fixed-rate arbitrary-cardinality mixed-radix scalar-product quantization | `saq-arbitrary-cardinality-feasibility-v2@b1a7429` | `AWAITING_AUTHORIZATION` | preserve terminal A4-1S `NO_GO_EXACT_SOLVER_COST` and reviewed A4-V2-PAR `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; CACHE-I remains `GENERIC_CACHE_POLICY_SOURCE_STATIC_REVIEW_PASS`; PREP authorization and both host-protocol governance stages remain documentation-only; after HOST-I-AUTH target/review `212a67b/@71e6bec`, the first HOST-I instruction stopped before a source target on the seven-versus-eight component contradiction, and additive source-authority erratum target/review `6fe8544/@9fa9528` reached only `HOST_I_SOURCE_AUTHORITY_ERRATUM_REVIEW_PASS`; the subsequently authorized 14-path HOST-I source-static target `4602585` was formed without Python, build, PREP, or data access, but its exact direct-child review `e8e9c79` failed with one HIGH because the binding and crosswalk cite a false SHA-256 for the governing erratum review; terminal source-target status remains `SOURCE_STATIC_TARGET_REVIEW_FAIL_AUTHORITY_IDENTITY_MISMATCH`; bounded correction-only repair protocol target/review `ddfef99/@b1a7429` then reached `HOST_I_R1_CORRECTION_ONLY_REPAIR_PROTOCOL_REVIEW_PASS`, freezing only a future exact five-path repair and review edge; actual `A4-V2-PREP-HOST-I-R1` is not authorized or executed, `HOST_IDENTITY_REBOUND` remains `NOT_ESTABLISHED`, and the separately disclosed `SOURCE_HISTORY_MISMATCH` remains unresolved and prevents cache-verifier/PAR-readiness claims; fresh PREP authorization, actual PREP, CACHE-BIND, corrected PAR, and scientific execution remain unauthorized | `UNREPORTED` | `ABSENT` | `INCLUDED_PENDING_REPORT`, slides 59--65 |
...
| SAQ | `saq-arbitrary-cardinality-feasibility-v2` | `b1a7429` | `FOLLOWUP` | `R12` | HOST-I source-static target/review `4602585/@e8e9c79` remains failed with one HIGH false governing-review digest; correction-only repair protocol target/review `ddfef99/@b1a7429` reached `HOST_I_R1_CORRECTION_ONLY_REPAIR_PROTOCOL_REVIEW_PASS`, but actual R1 is not authorized or executed, host identity is not rebound, and separate `SOURCE_HISTORY_MISMATCH` remains unresolved; Python/build/PREP/data were not run |
| SAQ | `saq-meeting-summary` | `SELF` -- commit containing this registry; parent at reconciliation `f002b57` | `SUMMARY` | all | summary-only; no experiment execution |
