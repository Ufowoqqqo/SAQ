...
| `R12` | 2026-07-13 draft Attempt 4 | Fixed-rate arbitrary-cardinality mixed-radix scalar-product quantization | `saq-arbitrary-cardinality-feasibility-v2@5db3025` | `AWAITING_AUTHORIZATION` | preserve terminal A4-1S `NO_GO_EXACT_SOLVER_COST` and reviewed A4-V2-PAR `ARTIFACT_INVALID / NO_SCIENTIFIC_DECISION`; the exact CACHE-I target `c33a2bf` and review `5db3025` reached `GENERIC_CACHE_POLICY_SOURCE_STATIC_REVIEW_PASS` for 37 filesystem sources and 38 executable units, but only as artifact governance; PREP authorization/review, clone preparation, receipt/review, CACHE-BIND, corrected PAR, and scientific execution remain unauthorized | `UNREPORTED` | `ABSENT` | `INCLUDED_PENDING_REPORT`, slides 59--65 |
...
| SAQ | `saq-arbitrary-cardinality-feasibility-v2` | `5db3025` | `FOLLOWUP` | `R12` | CACHE-I target `c33a2bf` and direct-child review `5db3025` reached `GENERIC_CACHE_POLICY_SOURCE_STATIC_REVIEW_PASS`; this is 37-source/38-unit artifact governance only, and PREP, CACHE-BIND, PAR-R1, SRUN, data, quarantine, and SAQ work remain unauthorized |
| SAQ | `saq-meeting-summary` | `SELF` -- commit containing this registry; parent at reconciliation `d1e8014` | `SUMMARY` | all | summary-only; no experiment execution |
