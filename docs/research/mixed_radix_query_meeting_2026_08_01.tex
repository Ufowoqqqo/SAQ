...
\begin{frame}{6. Closest Work Already Covers Most Primitives}
\footnotesize
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.24\textwidth} X}
Muresan--Effros & exact one-dimensional quantization with an arbitrary
integer number of reconstruction levels\\
Brandt transform coding & PCA, bit allocation, trained scalar quantizers,
packed words, and table-based ANN scanning\\
Adaptive Bit Allocation PQ & unequal integer bit budgets across independent
PQ subspaces\\
Finite Scalar Quantization & Cartesian products of scalar level sets,
including non-power-of-two factor sizes, mapped to one integer index\\
Quicker ADC & packed irregular-width PQ codes and the resulting lookup/SIMD
costs\\
PQ / OPQ / block VQ & more general subspace codebooks or learned space
decompositions under comparable payloads\\
\end{tabularx}
\begin{alertblock}{Important boundary}
No one paper is byte-for-byte identical to our implementation, but arbitrary
level counts, bit allocation, mixed-radix indexing, packed lookup, and learned
space decomposition are established primitives.
\end{alertblock}
\end{frame}

\begin{frame}{7. What Could Still Count as a Contribution?}
\small
\begin{block}{Not enough by itself}
A new combination of integer cardinalities, a lower reconstruction objective,
or a different packing layout is vulnerable to the objection that it directly
composes known quantization and lookup techniques.
\end{block}
\begin{alertblock}{The narrow remaining empirical claim}
Under the same database payload and a fully counted query consumer, arbitrary
cardinalities must produce a material, reproducible Recall--speed Pareto point
that strong PQ/OPQ and power-of-two controls do not reach.
\end{alertblock}
\begin{itemize}
  \item Count codebooks, radix metadata, table construction, lookup work,
  index build time, and memory---not payload bytes alone.
  \item Show that the gain comes from arbitrary cardinalities, not a stronger
  transform, coordinate grouping, optimizer, or extra query work.
  \item The following experiments test this narrow claim; they do not claim
  that the mixed-radix formula itself is new.
\end{itemize}
\end{frame}

\begin{frame}{8. What We Actually Compared}
\footnotesize
...
\begin{frame}{9. What the Natural-data Allocator Chose}
...
\begin{frame}{10. Natural-data Recall Result}
\small
...
\begin{frame}{11. Speed and Baseline Result}
\begin{itemize}
...
\begin{frame}{12. Why Add a Synthetic Positive Control?}
...
\begin{frame}{13. Synthetic Result: The Mechanism Reaches Recall}
\small
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
\begin{frame}{14. Put the Two Results Together}
\begin{center}
...
\begin{frame}{15. Coordinate Regrouping Was Not the Missing Contribution}
\small
...
perfect matching with our two-coordinate specialization of the Eigenvalue
Allocation rule in parametric OPQ (Ge et al., Section 3.2.4).
\begin{center}
\begin{tabular}{lrr}
\toprule
& search 64 cells & search 1,024 cells\\
\midrule
SIFT1M: matching minus 2D allocation control & +0.000253 & +0.000349\\
GIST1M: matching minus 2D allocation control & +0.000320 & -0.000160\\
\bottomrule
\end{tabular}
\end{center}
\begin{alertblock}{Interpretation}
Under our frozen two-coordinate scalar consumer, ``do not pair coordinates
mechanically'' is useful, but the tested OPQ-P-inspired control already
explains nearly all of the gain.  This is not a comparison with full OPQ.
Arbitrary integer radices still add at most about 0.00057 Recall.
\end{alertblock}
\end{frame}

\begin{frame}{16. Why Reconstruction Error Was Not Enough}
\footnotesize
...
\begin{frame}{17. Can Base Data Learn a Joint Encoder?}
\small
...
\begin{frame}[fragile]{18. Train on One Half, Test on the Other, Then Swap}
\small
...
\begin{frame}{19. Joint Selection Did Not Generalize}
\small
\begin{center}
\begin{tabular}{lrr}
\toprule
...
\begin{frame}{20. Final Claim Boundary}
...
\begin{itemize}
  \item Muresan and Effros. ``Quantization as Histogram Segmentation.''
  DCC 2002.
  \item Brandt. ``Transform Coding for Fast Approximate Nearest Neighbor
  Search.'' CVPR 2010.
  ...
  Section 3.2.4, ``Eigenvalue Allocation.''
  \item Guo et al. ``Adaptive Bit Allocation Product Quantization.''
  Neurocomputing 2016.
  \item Andr\'e et al. ``Quicker ADC.'' TPAMI 2021.
  \item Mentzer et al. ``Finite Scalar Quantization.'' ICLR 2024.
