...
\usepackage{xcolor}
\hypersetup{hidelinks,pdftitle={Mixed-radix: Natural-data Result and Synthetic Witness}}
\newcommand{\good}[1]{\textcolor{green!45!black}{\textbf{#1}}}
\newcommand{\bad}[1]{\textcolor{red!75!black}{\textbf{#1}}}
\title{Original Mixed-radix: What Worked and What Did Not}
\subtitle{A real mechanism, but no useful natural-data frontier improvement}
\author{}
\date{2026-08-01}
...
\begin{frame}{13. Claim Boundary and Next Question}
\begin{block}{What we can now say}
Fixed-word mixed-radix coding can preserve cardinalities that binary rounding
cannot, and this can prevent query-result collisions.
\end{block}
\begin{alertblock}{What we cannot say}
We have not shown that the condition is common in real vector data, that it
improves end-to-end QPS, or that it beats an unrestricted PQ/VQ model.
\end{alertblock}
\begin{block}{If we continue}
The next distinct question is whether base data alone can identify coordinate
groups whose quantization collisions affect near-neighbour ordering. First
compute an oracle upper bound; do not immediately build another large system.
\end{block}
\end{frame}
