...
Successful regeneration commands:
...
```text
The current matrix reproduction depends on local /tmp/saq-run artifacts.
A clean-machine regeneration path is not fully packaged yet.
```

Speaker notes:

- For meeting, the evidence table is durable.
- For paper, artifact regeneration needs more engineering.
...
clean-machine artifact regeneration still needs work
...
more datasets, clean-machine regeneration, QPS curves, or a stronger theory?
```
...
```text
package clean-machine regeneration path
add explicit missing-artifact checks
...
consider new local families only if they remain query-unaware and auditable
```
