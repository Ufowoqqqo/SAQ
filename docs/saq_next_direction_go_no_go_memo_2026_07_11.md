...
`CO-0A` was executed after this memo froze the gate and **failed**. The pinned
official Extended-RaBitQ `fast_quantize` routine misses a globally optimal
code on a reachable `D=64, B=3` input; its public path omits frozen-plan bit
widths `B=2,6,11`; and its byte output narrows legal `B=11` magnitudes.

Accordingly, the memo's conditional GO has expired as a **NO-GO under the
official-source contract**. `CO-0B` was not preregistered, no GIST/CIFAR output
was inspected, and no claim about finite-round CAQ on SAQ data was made. See
`docs/saq_caq_co0a_official_source_parity_2026_07_11.md` and its machine-readable
artifact for the frozen evidence.
