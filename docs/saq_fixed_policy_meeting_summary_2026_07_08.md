...
Update scope: this version keeps the 2026-07-07 method narrative and evidence
table intact. The new material reflects the 2026-07-08 reproducibility work,
the explicit SIGMOD/VLDB/ICDE-level contribution target, and the stronger
novelty/overhead caution.
...
The paper-level bar is higher than this empirical observation. A database
top-conference version would need to show that the policy exposes a concrete
SAQ limitation and that its extra candidate generation, scoring, build, and
search costs are justified by more than tiny local tuning gains.
...
## 7. Reproducibility And Input Provenance Update

The evidence table did not change on 2026-07-08. What changed is the artifact
story around it.
...
```text
The current matrix is no longer only a local /tmp result.
The input layer has full-file SHA256 identity checks.
The evaluated GIST, CIFAR, and DEEP rows have command-level preparation provenance.
The supported preparation and verification path is executable from one driver.
```

This improves meeting/paper defensibility, but it is not itself a new SAQ
method contribution.

## 8. Positive, Reject, And Abstention Cases
...
## 9. Implementation Fix, Not Method Contribution
...
- The input layer now has a full-SHA256 manifest and an executable
  preparation/verification driver, but a fresh-root run or archived input
  bundle is still needed before claiming a paper-grade artifact.
...
- The reproducibility layer strengthens the evidence boundary, but it does not
  solve the novelty concern by itself.

## 11. Meeting Questions

1. Is a shape-dependent fixed-policy correction layer around SAQ's default plan
   a worthwhile contribution direction for a SIGMOD/VLDB/ICDE-level paper?
...
3. Is the current reproducibility layer sufficient for meeting discussion, or
   should the next step be a fresh-root verification / archived-input bundle?
4. What stronger SAQ limitation or lower-overhead policy mechanism would make
   this more than a useful local correction layer?
