...
\begin{enumerate}
  ...
  \item padded-lane finite block-min search mode.
\end{enumerate}
...
These fixes are necessary for fair evaluation, but should not be presented as novelty.
\end{frame}
...
% ----------------------------------------------------------------------
\begin{frame}[fragile]{Review-Time Baseline Correction}
\scriptsize
...
\begin{tabular}{lrrrr}
\toprule
estimator & top1 disag. & mean rank & p90 rank & top8 \\
\midrule
unrotated symqg proxy & 0.87625 & 8.2775 & 19 & 0.61500 \\
review-time FHT reproduction & 0.2475--0.2975 & 1.4425--1.5600 & 2--3 & 0.99625--1.0000 \\
saq\_fast & 0.46125 & 2.2575 & 5 & 0.98125 \\
\bottomrule
\end{tabular}

\vspace{0.8em}
\begin{alertblock}{Interpretation}
The omitted transform may remove or reverse the apparent SAQ advantage. The
old positive comparison is provisional.
\end{alertblock}

This reproduction is also not a validated result: it is not a committed runner
and does not reproduce packed FastScan. It requires the source-aligned Phase 2
comparison before further method design.
\end{frame}
...
\begin{enumerate}
  \item Phase 1: correctness defaults and focused regression tests.
  \item Phase 2: source-aligned scalar estimator, then parity-tested packed path.
...
graph traversal compatibility & old unrotated proxy result is provisional; review-time FHT reproduction may match or dominate saq\_fast & active only as a source-aligned baseline-validation gate \\
\bottomrule
\end{tabularx}
\end{frame}
