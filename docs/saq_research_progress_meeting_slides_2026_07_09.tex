...
% ----------------------------------------------------------------------
\begin{frame}[fragile]{Source-Aligned Scalar And Packed Baseline}
\scriptsize
A committed scalar and packed implementation now matches pinned SymphonyQG
revision \texttt{6124ddb3...} field by field. The first GIST sample50k / K512 /
B=4 / subset-512 / seed-0 sanity run has 128 events:
...
symqg\_fht\_fastscan & 1024 & 0.304688 & 1.54688 & 3 & 0.968750 \\
...
The aligned baseline is much stronger than the old proxy and is stronger
than saq\_fast in this small setting. The old comparison is invalid as
comparative evidence.
\end{alertblock}

Release and ASAN parity cover packed bytes, LUTs, accumulators, factors,
distances, and tie ordering. Multiple estimates and traversal remain absent.
\end{frame}

% ----------------------------------------------------------------------
\begin{frame}[fragile]{Current Direction: Fixed-Seed Local Replay Gate}
\scriptsize
...
\begin{enumerate}
  ...
  \item Phase 2: completed; scalar and packed paths match pinned source.
...
graph traversal compatibility & scalar and packed SymphonyQG paths match; aligned baseline is stronger than saq\_fast on a small seed-0 sanity run; traversal absent & Phase 3 stop-gate evaluation next \\
\bottomrule
\end{tabularx}
\end{frame}
