...
## Result 2026-07-08: Fixed-policy artifact dependency boundary is explicit

### Claim

The fixed-policy matrix now has an explicit artifact-dependency review and a
read-only checker that separates clean-root prerequisites from artifacts the
runner can regenerate.
...
The current local root is complete for the fixed-policy matrix. On a clean
root, the critical prerequisite is not candidate/scorer/index/report artifacts,
because those can be regenerated by the runner; it is the dataset/PCA/IVF input
substrate. The fixed-policy runner still does not provide a one-command full
